...
describe.concurrent.each(models)('TaskGeneratorAgent - $name', ({
  name,
  model,
}) => {
  ...
  test.runIf(shouldRunAgentTests)(
    ...
    async () => {
      const messages = [
        {
          role: 'user' as const,
          content: 'Fix critical security bug, needs to be done by end of week',
        },
      ]
...
describe.concurrent.each(models)('BulkTaskGeneratorAgent - $name', ({
  name,
  model,
}) => {
  test.runIf(shouldRunAgentTests)(
  ...
  test.runIf(shouldRunAgentTests)(
    ...
    async () => {
      ...
      console.debug('Priority distribution:', Array.from(priorities).join(', '))
